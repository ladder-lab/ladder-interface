...
export function useV4Medal(chainId: ChainId) {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        const res = await Axios.get(v4Url + 'getTransferMedal', {
          chainId,
          address: account || ''
        })
        const data = res.data.data as MedalResp
        if (!data) {
          setResult(undefined)
          return
        }
...
export interface AccountRankValues {
  value: string
  rank: string | number
  account: string
}
...
export function useV4AccountVolumeRankTop(chainId: ChainId, timestamp: string) {
  const { account } = useActiveWeb3React()
  const [accountRank, setAccountRank] = useState<AccountRankValues>()
  const [result, setResult] = useState<AccountRankValues[]>()
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        const url = timestamp ? '' : 'getAccountVolumeRank'
...
export function useV4AccountAssetsRankTop(chainId: ChainId, timestamp: string) {
  const { account } = useActiveWeb3React()
  const [accountRank, setAccountRank] = useState<AccountRankValues>()
  const [result, setResult] = useState<AccountRankValues[]>()

  useEffect(() => {
    console.log('enter useV4AccountAssetsRankTop', timestamp)
    ;(async () => {
      if (!chainId) {
        setResult(undefined)
        setAccountRank(undefined)
      ...
      try {
        const url = timestamp == '0' ? 'getAccountAssetWeekRank' : 'getAccountAssetRank'
        const res = await Axios.get(v4Url + url, {
          chainId,
          address: account || '',
          pageSize: 10,
...
export function useV4AccountLiquidityRankTop(chainId: ChainId) {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        const res = await Axios.get(v4Url + 'getAccountTvlRank', {
          chainId,
          address: account || '',
          pageSize: 10,
          pageNum: 1
