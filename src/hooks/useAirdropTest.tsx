...
export function useAirdropData() {
  ...
  const [userBoxes, setUserBoxes] = useState<UserBoxes>({} as UserBoxes)
  const [userLuck, setUserLuck] = useState<UserLock>({} as UserLock)

  const getParticipants = useCallback(async () => {
    try {
      const res = await axiosAirdropInstance.get('/current-participants')
      setParticipants(res.currentParticipants)
    } catch (e) {
      console.error(e)
    }
  }, [])
  ...
  const getUserBoxes = useCallback(async () => {
    try {
      const res = await axiosAirdropInstance.get('/user-boxes', {
        params: { userAddress: account }
      })
      setUserBoxes(res.data as UserBoxes)
    } catch (e) {
      console.error(e)
    }
  }, [account])

  const getUserLock = useCallback(async () => {
    try {
      const res = await axiosAirdropInstance.get('/user-luck', {
        params: { userAddress: account }
      })
      setUserLuck(res.data as UserLock)
    } catch (e) {
      console.error(e)
    }
  }, [account])
  ...
  const cb = useCallback(() => {
    getUserBoxes()
    getUserLock()
  }, [account, getUserBoxes, getUserLock])*/
  ...
  // return { airdropData, refreshCb, participants, userBoxes, userLuck }
