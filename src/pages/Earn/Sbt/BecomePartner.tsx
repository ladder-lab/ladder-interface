...
function Users({ onSelected }: { onSelected: (type: number) => void }) {
  return (
    <TypeBox>
      ...
      <Button variant={'outlined'} onClick={() => onSelected(0)}>
        Selected
      </Button>
...
function Organization({ onSelected }: { onSelected: (type: number) => void }) {
  return (
    <TypeBox>
      ...
      <Button variant={'outlined'} onClick={() => onSelected(1)}>
        Selected
      </Button>
      <Box sx={{ position: 'absolute', bottom: '0px', right: '0px' }}>
        <House />
...
export default function BecomePartner() {
  ...
  return (
    <ContentWrapper>
      <HeadBox>
        <Box>
          <BlackText textAlign={'center'} fontSize={45} alignSelf={'center'}>
      ...
      <RelativeBox>
        <ContentBox
          sx={{ backgroundColor: '#FFFFFF', position: 'absolute', alignSelf: 'center', border: '1px solid #FC1D00' }}
        >
          <RowBox>
            <Users onSelected={setType} />
            <Organization onSelected={setType} />
          </RowBox>
          {type == 0 && <UsersForm />}
