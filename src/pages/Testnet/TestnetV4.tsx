...
import { useGetRemoteStep, useMakeTwitter } from '../../hooks/useVerifyTwitter'
...
const useTwitterOAuth = (afterToken?: () => void) => {
  ...
  const getOauthToken = useCallback(async () => {
    try {
      ...
      showModal(<MessageBox type="error">{e?.response?.data?.details || 'Failed to connect to Twitter'}</MessageBox>)
      console.warn('getOauthToken', e)
    }
    sessionStorage.setItem('twitter', JSON.stringify(account))
...
export default function TestnetV4() {
  ...
  const { sign } = useSignLogin(verifyAll)
  useEffect(() => {
    verifyAll()
  }, [account, verifyAll])
  ...
  useTwitterOAuth(verifyAll)
  useEffect(() => {
    if (isWindowVisible) {
      setTimeout(verifyAll, 2000)
    }
